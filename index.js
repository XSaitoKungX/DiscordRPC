...
function validateConfig() {
    ...
    } else if (rpc.details && rpc.details.length < 1) {
        console.error(ERROR(`Details provided does not meet the minimum character length of 1.`));
        return process.exit(0)
    }
    if (rpc.state && rpc.state.length > 128) {
        console.error(ERROR(`State provided exceeds the maximum character length of 128.`));
        return process.exit(0)
    } else if (rpc.state && rpc.state.length < 1) {
        console.error(ERROR(`State provided does not meet the minimum character length of 1`));
        return process.exit(0)
    }
    if (rpc.assets.largeImageText && rpc.assets.largeImageText.length > 128) {
        console.error(ERROR(`LargeImageText provided exceeds the maximum character length of 128.`));
        return process.exit(0)
    } else if (rpc.assets.largeImageText && rpc.assets.largeImageText.length < 1) {
        console.error(ERROR(`LargeImageText provided does not meet the minimum character length of 1.`));
        return process.exit(0)
    }
    if (rpc.assets.smallImageText && rpc.assets.smallImageText.length > 128) {
        console.error(ERROR(`SmallImageText provided exceeds the maximum character length of 128.`));
        return process.exit(0)
    } else if (rpc.assets.smallImageText && rpc.assets.smallImageText.length < 1) {
        console.error(ERROR(`SmallImageText provided does not meet the minimum character length of 1.`));
        return process.exit(0)
    }
    if (rpc.buttons.primary.label && rpc.buttons.primary.url) {
        if (rpc.buttons.primary.label.length > 128) {
            ...
            return process.exit(0)
        } else if (rpc.buttons.primary.label.length < 1) {
            console.error(ERROR(`Primary button label provided does not meet the minimum character length of 1.`));
            return process.exit(0)
        }
    } else {
